...
def _validate_repeat_interval(repeat_time_sec, default_value=60):
    """验证循环时间间隔的有效性
    
    Args:
        repeat_time_sec: 要验证的时间间隔值
        default_value: 默认值，当验证失败时返回
...
def get_api_info():
    ...
    repeat_time_sec = api_info.get('repeat_time_sec', 60)
    if not isinstance(repeat_time_sec, int) or repeat_time_sec <= 0:
        logger.warning("警告: API repeat_time_sec设置无效，请在配置文件中正确设置repeat_time_sec值")
        repeat_time_sec = 60
    
    logger.info(f"API配置已加载: addr={api_info['addr']}, repeat_time_sec={repeat_time_sec}秒")
    return api_info
...
class MCPBasicProber:
    ...
    async def main(self):
        ...
        # 获取并验证循环时间间隔
        repeat_time_sec = _validate_repeat_interval(self.api_info.get('repeat_time_sec', 60))
        if repeat_time_sec != self.api_info.get('repeat_time_sec', 60):
            self.logger.warning(f"警告: 配置的循环时间间隔无效，使用默认值{repeat_time_sec}秒")
